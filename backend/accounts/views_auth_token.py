"""
Auth views for UGA authentication.

Includes:
- AuthTokenView: JWT verification + local session creation
- TokenRefreshView: Refresh token rotation
- CurrentUserView: GET /api/accounts/me/ — returns current authenticated user
- LogoutView: POST /api/accounts/logout/ — clears auth cookies
...
class CurrentUserView(APIView):
    ...
    Auth is determined by the access_token HttpOnly cookie,
    which is read automatically by the CookieJWTAuthentication backend.

    Returns 401 if not authenticated (cookie missing, expired, or invalid).
    """
...
class LogoutView(APIView):
    ...
    Accepts both authenticated and unauthenticated requests so
    stale cookies can always be cleared.
    """
